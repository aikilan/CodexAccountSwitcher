...
#### Scenario: Legacy app support directory migrates once
- Given 旧目录 `~/Library/Application Support/CodexAccountSwitcher` 存在且新目录不存在
- When 应用初始化路径
- Then 旧目录必须整体迁移到 `~/Library/Application Support/LLMAccountSwitcher`
...
产品展示名称必须统一为 `LLM Account Switcher`。
