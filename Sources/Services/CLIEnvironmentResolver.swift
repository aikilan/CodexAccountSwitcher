...
struct CLIEnvironmentResolver: @unchecked Sendable {
    ...
    func resolveClaudeContext(
        ...
        case .claudeCompatible:
            ...
            let modelSettings = account.resolvedProviderModelSettings
            let availableModels = await availableModelsForBridge(
                account: account,
                providerBaseURL: provider.baseURL,
                apiKey: providerCredential.apiKey
            )
            let proxy = try await claudeCompatibleProviderProxyManager.prepareProxy(
                accountID: account.id,
            ...
            let resolvedProvider = ClaudeResolvedProvider(
                ...
                apiKeyEnvName: proxy.apiKeyEnvName,
                apiKey: proxy.apiKey,
                availableModels: availableModels,
                modelSettings: modelSettings
            )
            return ResolvedClaudeCLILaunchContext(
                ...
                providerSnapshot: ResolvedClaudeProviderSnapshot(
                    ...
                    modelSettings: resolvedProvider.modelSettings
                ),
    ...
    private func resolveOpenAICompatibleCodexEnvironment(
        ...
        let shouldUseBridge = shouldUseOpenAICompatibleCodexBridge(for: account)
        let availableModels = shouldManageModelCatalog || shouldUseBridge
    ...
    private func availableModelsForBridge(
        ...
        if let modelSettings = explicitProviderModelSettings(for: account) {
    ...
    private func explicitProviderModelSettings(for account: ManagedAccount) -> [ProviderModelSettings]? {
        guard let settings = account.providerModelSettings else {
            return nil
        }
        let normalized = ProviderModelSettings.normalized(settings, fallbackModel: account.resolvedDefaultModel)
    ...
    private func shouldManageCodexModelCatalog(for account: ManagedAccount) -> Bool {
        ...
        case .claudeCompatible:
            return true
        case .openAICompatible:
            return account.providerPresetID != "openai" || shouldUseOpenAICompatibleCodexBridge(for: account)
        case .githubCopilot:
            return true
        }
    }

    private func shouldUseOpenAICompatibleCodexBridge(for account: ManagedAccount) -> Bool {
        guard account.providerRule == .openAICompatible else {
            return false
        }
        if account.providerPresetID == ProviderCatalog.customPresetID {
            return true
        }
        if explicitProviderModelSettings(for: account) != nil {
            return true
        }
        return !account.supportsResponsesAPI
    }
    ...
    private func mergedCopilotAvailableModels(_ models: [String], defaultModel: String) -> [String] {
        mergedAvailableModels(
            models.compactMap(normalizedCopilotBridgeModelCandidate),
            defaultModel: normalizedCopilotBridgeModelCandidate(defaultModel) ?? "gpt-4.1"
        )
    }

    private func copilotLaunchConfiguration(
        for account: ManagedAccount,
        credential: CopilotCredential,
        status: CopilotAccountStatus?
    ) -> (model: String, availableModels: [String]) {
        let liveAvailableModels = (status?.availableModels ?? []).compactMap(normalizedCopilotBridgeModelCandidate)
        let currentModel = normalizedCopilotBridgeModelCandidate(status?.currentModel)
        let preferredModel = normalizedCopilotBridgeModelCandidate(account.defaultModel)
        let credentialModel = normalizedCopilotBridgeModelCandidate(credential.defaultModel)
        ...
        if let preferredModel, liveAvailableModels.isEmpty || liveAvailableModels.contains(preferredModel) {
            ...
            model = currentModel
        } else if let firstLiveModel = liveAvailableModels.first {
            model = firstLiveModel
        } else if let credentialModel {
    ...
    private func normalizedCopilotBridgeModelCandidate(_ model: String?) -> String? {
        let trimmedModel = model?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
