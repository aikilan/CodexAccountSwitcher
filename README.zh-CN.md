...
LLM Account Switcher 是一个原生 macOS 菜单栏工具，用来在一台机器上管理本地 LLM 账号工作区。本轮保留完整 Codex 能力，并露出 Claude 占位入口；它会集中管理本地账号信息，并通过原子更新 `~/.codex/auth.json` 来切换当前激活的 Codex 账号。
...
- 将应用元数据保存在 `~/Library/Application Support/LLMAccountSwitcher/accounts.json`，将凭据缓存保存在 `~/Library/Application Support/LLMAccountSwitcher/credentials-cache.json`，且不会触发钥匙串授权弹窗。
