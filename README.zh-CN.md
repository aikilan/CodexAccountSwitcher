...
Codex Account Switcher 是一个原生 macOS 菜单栏工具，用来在一台机器上管理多个 Codex 身份。它同时支持 ChatGPT 浏览器登录和 API Key 账号，把本地账号信息集中管理，并通过原子更新 `~/.codex/auth.json` 来切换当前激活账号。

## 功能

- 通过菜单栏工具和独立主窗口统一管理多个 Codex 账号。
...
- 将应用元数据保存在 `~/Library/Application Support/CodexAccountSwitcher/accounts.json`，将凭据缓存保存在 `~/Library/Application Support/CodexAccountSwitcher/credentials-cache.json`，且不会触发钥匙串授权弹窗。
